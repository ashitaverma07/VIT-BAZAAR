...
// Route to register the data of a new member into the DB
router.post("/", (req, res) =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    // Checking if the Email-ID has @vitbhopal.ac.in domain name
    const domain = email_id.substring(email_id.lastIndexOf("@") + 1);
    if (domain !== "vitbhopal.ac.in") {
      return res.status(400).json({
        error: "Bad Request: Only VIT institutional email ID's allowed.",
      });
    }
    try {
